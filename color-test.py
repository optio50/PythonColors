...
color_names = [k for k in vars(colors.fg) if not k.startswith('_')]

fg_inst = colors.fg()
bg_inst = colors.bg()
for count, name in enumerate(color_names):
    value1 = getattr(fg_inst, name)
    value2 = getattr(bg_inst, name)
    print(value1, colors.reverse, f"{name.ljust(22) + str(count).rjust(6)}", \
